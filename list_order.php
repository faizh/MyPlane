							...
							<table class="table1" style="color: black;">
								...
								<tbody>
									...
									require "action/koneksi.php";
										$username = $_GET["username"];
										$data = mysqli_query($connect, "SELECT * FROM costumer, rute where costumer.username='$username' && costumer.id_rute = rute.id_rute"); 
										$id = 1;
										while($d = mysqli_fetch_array($data)){
											$kode = $d["kode_reservasi"];
											echo '<tr height="80px">
											<td>'.$id.'</td>
											<td>'.$d["nama_pemesan"].'</td>
											<td>'.$d["tittle"].' '.$d["nama_penumpang"].'</td>
											<td>'.$d["asal"].'</td>
											<td>'.$d["tujuan"].'</td>
											<td> Rp '.$d["harga"].'</td>
